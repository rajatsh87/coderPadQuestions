...
public class TrainMapProblem {
   ...
   * is stored in the 'neighbours' field. Two station objects with the same name
   * are
  ...
  private static class TrainMap {
    ...
    public List<Station> shortestPath(String from, String to) {
      ...
      Map<String,Station> visitedStations = new HashMap<>();
      ArrayDeque<Station> deque = new ArrayDeque<>();
      if (this.stations.containsKey(from)) {
        visitedStations.put(this.stations.get(from).getName(),null);
        deque.add(this.stations.get(from));
        while(deque.size()>0){
          Station curr=deque.pop();
          for(Station st:curr.getNeighbours()){
            if(!visitedStations.containsKey(st.getName())){
              visitedStations.put(st.getName(),curr);
              deque.add(st);
            }
          }
        }
        String dest=to;
        ans.add(this.stations.get(dest));
        while(visitedStations.get(dest)!=null && visitedStations.containsKey(dest)){
          Station val=visitedStations.get(dest);
          ans.add(val);
          dest=val.getName();
        }
      }
  ...
  public static boolean doTestsPass() {
    ...
    return solution.equals(
        TrainMap.convertPathToStringRepresentation(trainMap.shortestPath("King's Cross St Pancras", "St Paul's")));
  }
